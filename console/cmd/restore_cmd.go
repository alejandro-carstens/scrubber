...
func (rc *restoreCmd) new(logger *logging.SrvLogger) *cobra.Command {
	...
	command.Flags().Int("max_wait", 3600, "specifies how long in seconds to wait to see if the action has completed before giving up")
	command.Flags().Int("wait_interval", 9, "specifies how long in seconds to wait to see if the action has completed before giving up")
